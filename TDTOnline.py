...
from app.pipeline import run_finetune_pipeline_from_paths
...
class OnlineGUI(tk.Tk):
    def __init__(self) -> None:
        ...
        self._predictor: Optional[SynapseSleepStagePredictor] = None
    ...
    def _build_prepare_tab(self) -> None:
        ...
        self._row_selector(frame, "MAT path (file or folder):", self.mat_path, self._choose_mat, 0)
        self._row_selector(frame, "TXT path (file or folder):", self.txt_path, self._choose_txt, 1)

        params = ttk.LabelFrame(self.prepare_tab, text="Key Parameters")
        params.pack(fill="x", padx=8, pady=8)
        self.eeg_channel = tk.StringVar(value="EEG_P")
        self.emg_channel = tk.StringVar(value="EMG_diff")
        ...
        ttk.Entry(params, textvariable=self.eeg_channel, width=16).grid(row=0, column=1, sticky="w", **pad)
        ttk.Label(params, text="EMG channel name").grid(row=0, column=2, sticky="w", **pad)
        ttk.Entry(params, textvariable=self.emg_channel, width=16).grid(row=0, column=3, sticky="w", **pad)
    ...
    # --------------------------- Dialogs
    def _choose_path(self, title: str, filetypes) -> Optional[str]:
        use_file = messagebox.askyesno("选择方式", "选择单个文件？\n选择“否”将选择文件夹。")
        if use_file:
    ...
    def _choose_mat(self) -> None:
        path = self._choose_path("Select MAT file or folder", [("MAT", "*.mat")])
        if path:
            self.mat_path.set(path)

    def _choose_txt(self) -> None:
        path = self._choose_path("Select TXT file or folder", [("TXT", "*.txt")])
        if path:
    ...
    def _start_online(self) -> None:
        ...
        def task() -> None:
            try:
                cfg = parse_runtime_config(runtime_path)
                if ckpt_path:
                    cfg.checkpoint_path = Path(ckpt_path)
                ...
                predictor.register_epoch_hook(hook)
                predictor.run()
            except Exception as exc:
