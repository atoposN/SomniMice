...
class APIStreamer:
    """
    Lightweight polling wrapper around SynapseAPI for streaming data buffers.
    Notes for TDTOnline setup:
    - Instantiate one streamer per Synapse gizmo (e.g., EEG gizmo `APIEEG`, EMG gizmo `APIEMG`).
    - Channel names map to Synapse parameters like `data1`, `data2`, ...; if EMG is on channels 3/4,
      configure them as `APIEMG.data3` and `APIEMG.data4` in `online_runtime.yaml`.
    - The online predictor uses dot-notation `Gizmo.Parameter` to resolve endpoints; keep gizmo/parameter
      names consistent with Synapse to avoid index errors in `_resolve_stream_channel_index`.
    """
    def __init__(self,
                host='localhost',
